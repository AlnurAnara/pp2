...
SPEED_INCREASE_INTERVAL = 10 # 金币增长速度区间
...
background = pygame.image.load("AnimatedStreet.png") # Load Background
...
coin_weights = [1, 2, 3]  
coin_probabilities = [0.7, 0.2, 0.1]  
...
while True:
    ...
    if random.random() < 0.02:  
        weight = random.choices(coin_weights, coin_probabilities)[0]
        C = Coin(weight)
        # Ensure coins don't spawn on the enemy's car
        while pygame.sprite.spritecollide(C, enemies, False):
    ...
    if COINS_COLLECTED >= SPEED_INCREASE_INTERVAL:
        SPEED += 2  # Increase enemy speed when player collects 10 coins
