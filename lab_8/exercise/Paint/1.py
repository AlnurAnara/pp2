...
def toggle_circle_mode():
    global circle_mode, rect_mode, eraser_mode
    circle_mode = True
    rect_mode = False
    eraser_mode = False

def toggle_rect_mode():
    global rect_mode, circle_mode, eraser_mode
    rect_mode = True
    circle_mode = False
    eraser_mode = False

def toggle_eraser_mode():
    global eraser_mode, circle_mode, rect_mode, brush_color
    eraser_mode = not eraser_mode  # 修复拼写错误
    circle_mode = False
    rect_mode = False
    if eraser_mode:
        brush_color = white  # 让橡皮擦变成白色
...
while True:
    ...
    if drawing:
        ...
        if mouse_y > 50:  # 避免绘制在按钮栏上
            if eraser_mode:
                pygame.draw.circle(canvas, white, (mouse_x, mouse_y), 10)  # 让橡皮擦生效
            else:
